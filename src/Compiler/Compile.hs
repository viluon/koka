...
import Backend.Lua.FromCore       ( luaFromCore )
...
codeGenLua :: Terminal -> Flags -> [Module] -> CompileTarget Type -> FilePath -> Core.Core -> IO (Maybe (FilePath, IO ()))
codeGenLua term flags modules compileTarget outBase core
  = do let outlua = outBase ++ ".lua"
       let mbEntry = case compileTarget of
                       Executable name tp -> Just (name,isAsyncFunction tp)
                       _                  -> Nothing
           extractImport mod = Core.Import (modName mod) (modPackageQName mod) Public ""
       let lua    = luaFromCore (buildType flags) mbEntry (map extractImport modules) core
       termPhase term ( "generate lua: " ++ outlua )
       writeDocW 80 outlua lua
       when (showAsmLua flags) (termDoc term lua)
         ...
         do -- always generate an index.html file
            let outHtml = outName flags ((if null (outBaseName flags) then "index" else (outBaseName flags)) ++ ".html")
                contentHtml = text $ unlines [
                                ...
                                "  <head>",
                                "    <meta charset=\"utf-8\">",
                                "    <script type='module' src='./" ++ notdir outlua ++ "'></script>",
                                "  </head>",
                                "  <body>",
                                "  </body>",
                                "</html>"
            ...
            termPhase term ("generate index html: " ++ outHtml)
            writeDoc outHtml contentHtml
            return (Just (outHtml, runSystemEcho term flags (dquote outHtml ++ " &")))
...
copyIFaceToOutputDir term flags iface core
  ...
  = do let outIFace = outName flags (notdir iface)
       ...
       case target flags of
        ...
        Lua
          -> do let luaSrc = notext iface ++ ".lua"
                let luaOut = notext outIFace ++ ".lua"
                copyTextIfNewer (rebuild flags) luaSrc luaOut
        C _
